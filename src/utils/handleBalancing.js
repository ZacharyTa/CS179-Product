...
function validateMoves(currNode, container) {
  //this is the container in grid currently
  var moves = [];
  ...
  for (var i = 0; i < grid.length; i++) {
    for (var j = 0; j < grid[i].length; j++) {
      ...
      if (
        ...
        var no_containerTop =
          i === grid.length - 1 || grid[i + 1][j].name === "UNUSED";
        if (no_containerTop) {
    ...
    for (var i = 0; i < buffer.length; i++) {
      for (var j = 0; j < buffer[i].length; j++) {
        ...
        if (
          container &&
          buffer[i][j].name !== "NAN" &&
          buffer[i][j].name !== "UNUSED"
        ) {
          var noContainerTop =
            i === buffer.length - 1 || buffer[i + 1][j].name === "UNUSED";
...
function heuristic(child) {
  ...
  for (var move of moves) {
    //if enabled, then some ms are buffers!
    for (var m of move.moves) {
      ...
      if (enable === true) {
        ...
        var newWeightDiff = Math.abs(
          newWeights.left_weight - newWeights.right_weight,
        );
...
function isSolvable(ship) {
  ...
  if (weights.length >= 60) {
    //might need to change this later, but for now will only enable when more
...
function SIFT(ship) {
  //for the sake of testing that it returns []
  return [];
}
...
export default function handleBalancing(manifestText) {
  ...
  while (!frontier.isEmpty()) {
    ...
    if (checkBalance(weights) && currNode.problem.bufferEmpty()) {
      //goal reached
      ...
      var end =
        Math.abs(8 - lastElement.newRow) + Math.abs(0 - lastElement.newColumn);
      console.log("end: ", end);
      ...
      for (var m of moves) {
        for (var i of m.moves) {
          ...
          if (enable === true) {
            ...
            //if within buffer only
            if (i.type === "buffer2") {
              var grids = bridgeMoves(
                currNode.problem.buffer,
                currNode.problem.buffer,
                i,
              );
          ...
          if (currNode.cost === 0) {
            //when crane should start at initial position
            ...
            //time from prev container to new container
            ...
            if (
              !cm ||
              (cm.name !== mm.name &&
                (mm.newRow !== i.oldRow || mm.newColumn !== i.oldColumn))
            ) {
          ...
          var newCost = currNode.cost + 0.5 * i.cost;
          +0.001 * craneTime;
          var child = new Node(
            newP,
            currNode,
