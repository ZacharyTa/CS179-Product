      ...
      if (buffer[i][j].name === "UNUSED") {
        ...
        if (parent.move == null || parent.move.newRow == 9) {
          ...
          if (parent.newCol - 1 < 0) {
            dist +=
              Math.abs(parent.newRow - 4) +
              Math.abs(parent.newCol - 0) +
              4 +
              Math.abs(row - 8) +
              Math.abs(col - 0);
          }
    ...
    if (type == "offload") {
      ...
      if (parent.move != null) {
        ...
        if (parent.move.type == "buffer") {
          dist +=
            Math.abs(parent.move.newRow - 4) +
            Math.abs(parent.move.newColumn - 0) +
            4 +
            Math.abs(8 - i) +
            Math.abs(0 - j);
        } else {
...
function expandNode(curr, operations, frontier, visited) {
  ...
  for (let i = ship.length - 1; i >= 0; i--) {
    ...
    for (let j = 0; j < ship[i].length; j++) {
      ...
      if (i != ship.length - 1 && ship[i + 1][j].name != "UNUSED") {
        continue;
      }
  ...
  for (let j = 0; j < 24; j++) {
    for (let i = buffer.length - 1; i >= 0; i--) {
      ...
      if (curr.move == null || curr.move.newRow == 8) {
        ...
        curr.move != null &&
        (curr.move.type == "offload" || curr.move.type == "onload")
      ) {
        dist += 2;
      }
      let move = {
...
export default function handleLoading(manifestText, operations) {
  ...
  let buffer = Array.from({ length: 4 }, () =>
    new Array(24).fill({ w: 0, name: "UNUSED" }),
  );
  ...
  return solutionPath.map((move) => ({
    ...
    oldColumn:
      move.type == "buffer" &&
      (!Object.is(-0, move.newColumn) || move.oldColumn < 0)
        ? move.oldColumn - 1
        : move.oldColumn + 1, //
    newRow: move.newRow + 1,
    newColumn:
      move.type == "buffer" &&
      (Object.is(-0, move.newColumn) || move.newColumn < 0)
        ? move.newColumn - 1
        : move.newColumn + 1,
  }));
}
